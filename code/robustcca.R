...
Sigmaxy<-Sigmax%*%vx%*%t(vy)%*%Sigmay*lambda1
...
Resultrho[kk,1]<-lambda1-rhof(evx,evy,Sigmax,Sigmay,Sigmaxy)
Resultrho[kk,2]<-lambda1-rhof(vx1,vy1,Sigmax,Sigmay,Sigmaxy)
Resultrho[kk,3]<-lambda1-rhof(esvx,esvy,Sigmax,Sigmay,Sigmaxy)
...
Res1[ni+3*(di-1),(1:3)+3*(mi-1)]<-colMeans(Resultrho*100)
...
#write.table(round(Res1,1),"ccarho.txt",sep="&")
#write.table(round(Res2,1),"ccal.txt",sep="&")
#write.table(round(Res3,1),"ccasle.txt",sep="&")
